        ...
        public function checkIntegrity()
        {
            ...
            foreach ($this->classes as $name => $class) {
                if (
                    ...
                    &&
                    (
                    class_exists("\\Business\\" . $class->getName(), true)
                    )
                ) {
                    ...
                    $info = new \ReflectionClass("Business\\" . $name);
                    ...
                    if ($info->implementsInterface("OnPhp\\Prototyped")) {
                        $this->checkClassSanity(
                            $class,
                            new \ReflectionClass("Proto\\Proto" . $name)
                        );
                    }
                    ...
                    if ($info->implementsInterface("OnPhp\\DAOConnected")) {
                        $this->checkClassSanity(
                            $class,
                            new \ReflectionClass('DAOs\\' . $name . 'DAO')
                        );
                    }
                    ...
                    $objectName = "Business\\" . $name;
                    $object = new $objectName;

                    $proto = $object->proto();
                    $form = $proto->makeForm();

                    /** yet I do not understand why this test is necessary */
